...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        logger.error("数据库初始化失败，拒绝启动")
        raise
    
    # 旧凭据不得悄悄作为明文继续使用；迁移必须在停机维护窗口显式执行。
    from scripts.migrate_credentials import migrate
    from common.db.session import async_engine
    async with async_engine.connect() as connection:
        await connection.run_sync(lambda conn: migrate(conn, verify_only=True))

    # 从独立部署密钥派生JWT密钥，禁止弱默认值回退。
    try:
        from app.services.jwt_secret_service import ensure_jwt_secret_key
        await ensure_jwt_secret_key(settings)
    except Exception as e:
        logger.error("JWT 密钥自检失败，拒绝启动")
        raise

    # 所有后端通过独立部署密钥派生同一内部令牌。
    try:
...
async def global_exception_handler(request, exc):
    ...
    return JSONResponse(
        ...
        content={
            "success": False,
            "code": 500,
            "message": public_error_message or "服务器内部错误，请查看服务端日志",
            "data": None,
        },
    )
