...
router = APIRouter(prefix="/password-login", tags=["密码登录"], dependencies=[Depends(require_internal_auth)])
        ...
        try:
            async def _do_save(session_maker):
                async with session_maker() as session:
                    ...
                    require_same_account_identity(existing_account, unb)
...
async def check_login_status(session_id: str, user_id: int = Query(..., gt=0)):
    """
    检查账号密码登录状态
    ...
    try:
        ...
        if session_id not in password_login_sessions or password_login_sessions[session_id].get("user_id") != user_id:
...
async def cancel_login(session_id: str, user_id: int = Query(..., gt=0)):
    """取消登录会话"""
    try:
        if session_id not in password_login_sessions or password_login_sessions[session_id].get("user_id") != user_id:
            return {
