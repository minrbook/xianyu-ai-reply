...
class YifanApiHandler:
    ...
    async def get_yifan_api_card_content(self, rule, order_id=None, item_id=None, buyer_id=None, chat_id=None):
        ...
        try:
            ...
            api_url = str(api_config.get("api_url") or "").strip()
            from common.utils.remote_security import require_trusted_remote
            require_trusted_remote(api_url)
            if callback_url:
                require_trusted_remote(callback_url)
            
            timeout_obj = aiohttp.ClientTimeout(total=30)
            async with self.session.post(api_url, data=params, timeout=timeout_obj, allow_redirects=False) as response:
