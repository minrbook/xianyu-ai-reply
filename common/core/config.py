...
class BaseConfig(BaseSettings):
    ...
    # 默认关闭；启用时也不记录SQL字面量或参数。
    sql_echo: bool = Field(default=False)

    credential_encryption_key_file: str = Field(default="", alias="CREDENTIAL_ENCRYPTION_KEY_FILE", repr=False)
    remote_credential_origins: str = Field(default="", alias="REMOTE_CREDENTIAL_ORIGINS")
    ...
    # 运行期JWT密钥由独立部署密钥按服务用途派生，不能使用此占位默认值。
    ...
    # 内部令牌在启动时从独立部署密钥派生，不使用数据库或环境中的旧值。
