...
_SENSITIVE_LOG = re.compile(
    r"(?i)(?:cookie(?:s|_str)?|password|authorization|api[_-]?key|secret[_-]?key|"
    r"access[_-]?token|refresh[_-]?token|sgcookie|_m_h5_tk|sign_string)[\"']?\s*[:=]"
)

def redact_log_record(record):
    message = record.get("message", "")
    if _SENSITIVE_LOG.search(message):
        record["message"] = "[敏感日志内容已隐藏]"
    exc = record.get("exception")
    if exc and _SENSITIVE_LOG.search(str(exc.value)):
        record["exception"] = None
        record["message"] = "[敏感异常内容已隐藏]"
...
def _add_file_handlers(log_file: Path, retention_days: int) -> tuple[int, int]:
    ...
    """
    # Shared hosts cap archive count as well as each file's size.
    archive_limit = os.environ.get("LOG_FILE_MAX_ARCHIVES")
    retention = max(1, min(10, int(archive_limit))) if archive_limit else f"{retention_days} days"
