        ...
        try:
            async with async_session_maker() as session:
                ...
                result = await session.execute(
                    text("SELECT id, password_hash FROM xy_users WHERE username = 'admin' LIMIT 1")
                )
                ...
                if existing:
                    ...
                    if verify_password("admin123", existing.password_hash):
                        raise RuntimeError("管理员仍使用旧默认密码，必须先更换密码再启动")
                ...
                if not password_file:
                    raise RuntimeError("首次启动必须配置 INITIAL_ADMIN_PASSWORD_FILE")
                password = Path(password_file).read_text().strip()
                if len(password) < 16 or password.lower() in {"admin123", "change-me-in-production"}:
                    raise RuntimeError("初始管理员密码必须至少16位，推荐随机生成")
                ...
                logger.info("初始密码从部署 secret 文件读取，不写入日志")
                
        except IntegrityError:
            logger.info("✓ 管理员用户已存在，跳过创建")
        except Exception as e:
            logger.error("创建管理员失败，拒绝启动")
            raise
