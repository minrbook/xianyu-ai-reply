        ...
        try:
            async with async_session_maker() as session:
                ...
                if clean_expected_token:
                    # 随机化密文不能用于等值查询；锁定记录后比较解密值，避免误伤新Token。
                    row = (await session.execute(
                        select(TokenCache).where(TokenCache.user_id == clean_user_id).with_for_update()
                    )).scalar_one_or_none()
                    if row is None or row.token != clean_expected_token:
                        return TokenCacheInvalidationResult(True, changed=False, message="Token已更新或不存在")
                    conditions.append(TokenCache.id == row.id)
